...
$ vrs-annotate omop output.vcf.gz --vcf_out omop-annotated-output.vcf.gz
...
from ga4gh.vrs.extras.annotator.omop import OMOPAnnotator
...
@click.option(
    ...
    required=False,
    type=str,
    default="http://localhost:8000/api/odmapper",
    help="The base URL for ODmapper"
)
@click.option(
    "--silent",
    "-s",
...
def _annotate_omop_cli(
    vcf_in: Path,
    vcf_out: Path | None,
    vrs_pickle_out: Path | None,
    odmapper_base_url: str,
    silent: bool,
) -> None:
    """Annotate OMOP concept_id from VRS_ALLELE_IDs located at VCF_IN.
    ...
    annotator = OMOPAnnotator(odmapper_base_url)
    start = timer()
    msg = f"Annotating {vcf_in} with the VCF OMOP Annotator..."
    _logger.info(msg)
    if not silent:
        click.echo(msg)
    annotator.annotate(
        ...
        output_vcf_path=vcf_out,
        output_pkl_path=vrs_pickle_out,
        odmapper_base_url=odmapper_base_url,
    )
    end = timer()
    msg = f"VCF OMOP Annotator finished in {(end - start):.5f} seconds"
    _logger.info(msg)
    if not silent:
        click.echo(msg)
