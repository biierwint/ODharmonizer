...
class ConceptStandardView(APIView):
    """
    Given a concept_id, resolve to its standard concept_id using 'Maps to' relationship.
    """
    def get(self, request, concept_id):
        try:
            original_concept = Concept.objects.get(concept_id=concept_id)
        except Concept.DoesNotExist:
            return Response({"error": "Concept not found."}, status=status.HTTP_404_NOT_FOUND)

        mapped_relationship = ConceptRelationship.objects.filter(
            concept_id_1=original_concept,
            relationship_id='Maps to'
        ).first()

        if mapped_relationship:
            standard_concept = mapped_relationship.concept_id_2
        else:
            standard_concept = original_concept

        return Response({
            "original_concept_id": original_concept.concept_id,
            "standard_concept_id": standard_concept.concept_id,
            "standard_concept_name": standard_concept.concept_name,
            "standard_vocabulary_id": standard_concept.vocabulary_id,
            "standard_concept_class_id": standard_concept.concept_class_id
        })
